	  ...
	  D.qr('index.php?act=show-properties', {'items': window.items}, function(res,err)
	  {
	   ...
	   window.sync_mods = function(smart2numeric /* 'smart->numeric' = true or 'numeric->smart' = false */)
	   {
		   if(smart2numeric)
		   {
			   ...
			   {
					mask = Math.pow(2,i);
					if($('mod_smart['+mask+']').checked) oct_mod += mask;
			   }
		   ...
		   }else
		   {
			   ...
			   {
					mask = Math.pow(2,i);
					$('mod_smart['+mask+']').checked = oct_mod & mask;
			   }
		   }
	   }
	   ...
	   var box = function(smask)
	   {
		...
		<tr bgcolor="#dedede">\
			...
			<button onclick="E.ci($(\'mod\').value,items);"><b>chmod!</b></button>\
		</th>\
		</td>\
		\
		</table>\
