...
function get_info($f, $_REQUEST = array())
{
	...
	}else if(in_array($_REQUEST['type'],array(tDIR,tFILE)))
	{
		...
		if(preg_match('/Darwin/is', PHP_OS)) $imgext = array_merge($imgext, explode(' ','pdf doc docx ppt pptx xls xlsx tiff bmp mov avi mpg mp4 tga'));
		...
		'id3' => (($img && d_filesize($f) < 5*1024*1024 && @$sz=getimagesize($f)) ? 'Dimensions: '.$sz[0].'x'.$sz[1] : ''),
		'owner' => get_owner($f),
		'group' => get_group($f),
