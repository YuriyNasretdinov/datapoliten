...
<head>
	...
	function send_changes(clos)
	{
		...
		Dolphin.qr('<?=dirname($_SERVER['REQUEST_URI']).'/'?>index.php?act=save-file&charset='+$('charset').value, { filename_encoded: "<?=rawurlencode($f)?>", content: cont }, function(res,err)
		{
			...
			{
				$('changes').innerHTML = 'File has been saved successfully.';
				$('save').disabled = false;
			}
...
<body onbeforeunload="if(cont_hash!=count_hash()) return 'Leave unsaved changes?';" onkeydown="if(event.ctrlKey && (event.charCode && event.charCode==115||event.keyCode==83) /* s */) { if(event.returnValue) event.returnValue=false; if(event.preventDefault) event.preventDefault(); if(event.stopPropagation) event.stopPropagation(); if(!$('save').disabled) $('save').onclick(event); return false; }"><div id="loading">&nbsp;</div><table height="100%" width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr height="40">
		<td>
