...
function send_thumbnail($fullpath,$width=80,$height=60,$resample=-1)
{
    ...
    $c = 2; /* coeffitient for fast image resampling */
...
/* set decimal $mod to $file
   $rem -- remember CHMOD value and return them to initital state at the end of script work?
...
function multipart_cont($cache)
{
	...
	while($it = array_pop($files))
	{
		...
		if(is_file($name) || $a == 'd' || $a == 'i')/*only execute processors*/ 
		{
